...
def exp_map_SO3xR3(tangent_vector: jt.Var) -> jt.Var:
    ...
    rot_angles = jt.clamp(norms, 1e-4).sqrt()  # B, N
    ...
    skews = jt.zeros(tangent_vector.shape[:-1] + (3, 3), dtype=log_rot.dtype)  # B, N, 3, 3
    # fmt: off
    skews[..., 0, 1] = -log_rot[..., 2]
    skews[..., 0, 2] =  log_rot[..., 1]
    skews[..., 1, 0] =  log_rot[..., 2]
    ...
    ret = jt.zeros(tangent_vector.shape[:-1] + (3, 4), dtype=tangent_vector.dtype)  # B, N, 3, 4
    ide = init.eye(3, dtype=log_rot.dtype) # 3, 3
...
def exp_map_SE3(tangent_vector: jt.Var) -> jt.Var:
    ...
    theta = jt.norm(tangent_vector_ang, dim=1).unsqueeze(1)
    theta2 = theta**2
    theta3 = theta**3

    near_zero = theta < 1e-2
    non_zero = jt.ones(1, dtype=tangent_vector.dtype)
    theta_nz = jt.ternary(near_zero, non_zero, theta)
    theta2_nz = jt.ternary(near_zero, non_zero, theta2)
    theta3_nz = jt.ternary(near_zero, non_zero, theta3)

    # Compute the rotation
    sine = theta.sin()
    cosine = jt.ternary(near_zero, 8 / (4 + theta2) - 1, theta.cos())
    sine_by_theta = jt.ternary(near_zero, 0.5 * cosine + 0.5, sine / theta_nz)
    one_minus_cosine_by_theta2 = jt.ternary(near_zero, 0.5 * sine_by_theta, (1 - cosine) / theta2_nz)
    ret = jt.zeros(tangent_vector.shape[0], 3, 4).to(dtype=tangent_vector.dtype)
    ...
    # Compute the translation
    sine_by_theta = jt.ternary(near_zero, 1 - theta2 / 6, sine_by_theta)
    one_minus_cosine_by_theta2 = jt.ternary(near_zero, 0.5 - theta2 / 24, one_minus_cosine_by_theta2)
    theta_minus_sine_by_theta3_t = jt.ternary(near_zero, 1.0 / 6 - theta2 / 120, (theta - sine) / theta3_nz)

    ret[:, :, 3:] = sine_by_theta * tangent_vector_lin
    ret[:, :, 3:] += one_minus_cosine_by_theta2 * jt.cross(tangent_vector_ang, tangent_vector_lin, dim=1)
