...
class TSDF:
    ...
    def from_file(cls, tsdf_file):
        ...
        tsdf_values = jt.Var(tsdf_data['tsdf_values'])
        origin = jt.Var(tsdf_data['origin'])
        voxel_size = tsdf_data['voxel_size'].item()

        tsdf_weights = jt.ones_like(tsdf_values)
    ...
    def from_bounds(cls, bounds: dict, voxel_size: float):
        ...
        origin = jt.float([bounds['xmin'], bounds['ymin'], bounds['zmin']])

        voxel_coords = cls.generate_voxel_coords(
            origin, (num_voxels_x, num_voxels_y, num_voxels_z), voxel_size).half()

        # init to -1s
        tsdf_values = -jt.ones_like(voxel_coords[0]).half()
        tsdf_weights = jt.zeros_like(voxel_coords[0]).half()
    ...
    def generate_voxel_coords(cls,
        ...
        grid = jt.meshgrid([jt.arange(vd) for vd in volume_dims])

        voxel_coords = origin.view(3, 1, 1, 1) + jt.stack(grid, 0) * voxel_size

        return voxel_coords

    def cuda(self):#?????
        """ Moves TSDF to gpu memory. """
        self.voxel_coords = self.voxel_coords
        self.tsdf_values = self.tsdf_values
        if self.tsdf_weights is not None:
            self.tsdf_weights = self.tsdf_weights
...
class TSDFFuser:
    ...
    def __init__(self, tsdf, min_depth=0.5, max_depth=5.0, use_gpu=True):
        ...
        self.hom_voxel_coords_14hwd = jt.cat(
            (self.voxel_coords, jt.ones_like(self.voxel_coords[:1])), 0).unsqueeze(0)
    ...
    def project_to_camera(self, cam_T_world_T_b44, K_b44):
        ...
        if self.use_gpu:
            cam_T_world_T_b44 = cam_T_world_T_b44
            K_b44 = K_b44
            self.hom_voxel_coords_14hwd = self.hom_voxel_coords_14hwd

        world_to_pix_P_b34 = jt.matmul(K_b44, cam_T_world_T_b44)[:, :3]
        batch_size = cam_T_world_T_b44.shape[0]

        world_points_b4N = \
            self.hom_voxel_coords_14hwd.expand(batch_size, 4, *self.shape).flatten(start_dim=2)
        cam_points_b3N = jt.matmul(world_to_pix_P_b34, world_points_b4N)
    ...
    def integrate_depth(
        ...
        img_size = jt.float16([img_w, img_h]).view(1, 1, 1, 2)
        if self.use_gpu:
        ...
        if depth_mask_b1hw is not None:
            ...
            depth_b1hw[depth_mask_b1hw.logical_not()] = -1

        # Sample the depth using grid sample
        sampled_depth_b1hw = nn.grid_sample(input=depth_b1hw,
        ...
        tsdf_vals_b1N = jt.clamp(dist_b1N / self.truncation, min=-1.0, max=1.0)
        ...
        for tsdf_val_1N, valid_points_1N, confidence_1N in zip(tsdf_vals_b1N,
            ...
            update_rate = jt.ternary(confidence < old_weights, 2.0, 5.0).half()
            ...
            self.tsdf_weights[valid_points_hwd] = jt.clamp(total_weights, max=1.0)
