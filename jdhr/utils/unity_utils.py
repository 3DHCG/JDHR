...
def c2w_unity2opencv(c2w: np.ndarray):
    ...
    Returns:
        c2w: (np.ndarray), (4, 4), camera to world matrix in jdhr's OpenCV convention.
    """
    c2w = c2w.copy()
...
def trans_unity2opencv(t: np.ndarray):
    ...
    Returns:
        t: (np.ndarray), (3,), translation vector in jdhr's OpenCV convention.
    """
    t = t.copy()
...
def unity_qt2opencv_w2c(quanternion: np.ndarray,
    ...
    """ Decode the quaternion vector and translation vector from Unity into w2c.
        This is a simplest conversion function only changes from Unity to jdhr.
    Args:
...
#         w2c: (jt.Var), (4, 4), world to camera matrix.
#     """
#     # Convert the translation vector into jdhr convention first
...
# A version from the w2c perspective, which should make the jdhr
...
def decode_single_unity_proj_mats(k1, k2, k3, k4, H, W):
    """ Decode the projection matrix of Unity to camera intrinsic of jdhr.
    Args:
    ...
    Returns:
        K: (jt.Var), (3, 3), the camera intrinsic matrix of jdhr.
    """
    fx = k1 * W / 2.
    cx = (1 - k2) * W / 2.
    fy = k3 * H / 2.
...
def encode_jdhr_stereo_imgs(tensor_left: jt.Var, tensor_right: jt.Var):
    """ Encode the jdhr rendered stereo image pairs using `turbojpeg`, which is
