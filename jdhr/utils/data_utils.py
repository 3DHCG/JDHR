...
def get_mesh(verts: jt.Var, faces: jt.Var, uv: jt.Var = None, img: jt.Var = None, colors: jt.Var = None, normals: jt.Var = None, filename: str = "default.ply"):
    ...
    from jdhr.utils.mesh_utils import face_normals, loop_subdivision
...
def get_rigid_transform(pose: np.ndarray, joints: np.ndarray, parents: np.ndarray):
    ...
    # joints: N, 3
    # parents: N
    from jdhr.utils.blend_utils import get_rigid_transform_nobatch as net_get_rigid_transform
...
def get_near_far(bounds, ray_o, ray_d) -> Tuple[np.ndarray, np.ndarray]:
    ...
    from jdhr.utils.ray_utils import get_near_far_aabb
...
def affine_inverse(m: np.ndarray):
    from jdhr.utils.math_utils import affine_inverse
...
def load_resize_undist_ims_bytes(ims: np.ndarray,
    ...
    # from jdhr.utils.dist_utils import get_rank
    # if not get_rank(): __import__('jdhr.utils.console_utils', fromlist=['debugger']).debugger()
    # else:
    #     while 1: pass
...
def decode_crop_fill_im_bytes(im_bytes: BytesIO,
    ...
    H, W, _ = img.shape
    from jdhr.utils.bound_utils import get_bound_2d_bound
