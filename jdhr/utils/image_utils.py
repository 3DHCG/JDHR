...
def crop_using_mask(msk:jt.Var,
                    K:jt.Var,
                    *list_of_imgs: List[jt.Var]):
    ...
    K = jt.stack(K)  # stack source dim
    # Resize instead of filling things up?
    # Filling things up might be easier for masked output (bkgd should be black)
    H_max = max(hs)
    W_max = max(ws)
    list_of_imgs = [jt.stack([fill_nhwc_image(im, size=(H_max, W_max)) for im in img]) for img in list_of_imgs]  # HACK: evil list comprehension
...
def crop_using_xywh(x, y, w, h, K, *list_of_imgs):
    K = K.clone()
    K[..., :2, -1] -= jt.array([x, y])  # crop K torch.as_tensor
    list_of_imgs = [img[..., y:y + h, x:x + w, :]
                    if isinstance(img, jt.Var) else
                    [im[..., y:y + h, x:x + w, :] for im in img]  # HACK: evil list comprehension
                    for img in list_of_imgs]
    return K, *list_of_imgs


def pad_image_to_divisor(img:jt.Var, div: List[int], mode='constant', value=0.0):
    H, W = img.shape[-2:]
...
def pad_image(img:jt.Var, size: List[int], mode='constant', value=0.0):
    ...
    if Wt >= W and Ht >= H:
        img = nn.pad(img, pad, mode, value)
    else:
        if Wt < W and Ht >= H:
            img = nn.pad(img, (0, 0, 0, Ht - H), mode, value)
        if Wt >= W and Ht < H:
            img = nn.pad(img, (0, Wt - W, 0, 0), mode, value)
        img = img[..., :Ht, :Wt]
...
def fill_nchw_image(img:jt.Var, size: List[int], value: float = 0.0, center: bool = False):
...
def fill_nhwc_image(img:jt.Var, size: List[int], value: float = 0.0, center: bool = False):
    bs = img.shape[:-3]  # -3, -2, -1
    cs = img.shape[-1:]
...
def interpolate_image(img:jt.Var, mode='bilinear', align_corners=False, *args, **kwargs):
    # Performs F.interpolate as images (always augment to B, C, H, W)
    sh = img.shape
    img = img.view(-1, *sh[-3:])
    img = nn.interpolate(img, *args, mode=mode, align_corners=align_corners if mode != 'nearest' else None, **kwargs)
    img = img.view(sh[:-3] + img.shape[-3:])
    return img


def resize_image(img:jt.Var, mode='bilinear', align_corners=False, *args, **kwargs):
    sh = img.shape
