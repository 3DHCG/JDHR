...
class Meshes:
    ...
    def __init__(
        ...
        *,
        ...
        if isinstance(verts, list) and isinstance(faces, list):
            ...
            self._faces_list = [
                f[f.gt(-1).all(1)].to(jt.int64) if len(f) > 0 else f for f in faces
            ]
            self._N = len(self._verts_list)
            self.valid = jt.zeros((self._N,)).boll()
            if self._N > 0:
                ...
                #    )
                self._num_verts_per_mesh = jt.Var(
                    [len(v) for v in self._verts_list]
                )
                self._V = int(self._num_verts_per_mesh.max())
                self._num_faces_per_mesh = jt.Var(
                    [len(f) for f in self._faces_list]
                )
        ...
        elif jt.is_var(verts) and jt.is_var(faces):
            ...
            self._faces_padded = faces.to(jt.int64)
            ...
            if self._N > 0:
                ...
                self._num_verts_per_mesh = jt.full(
                    size=(self._N,),
                    val=self._V,
                    dtype=jt.int64,
                )
        ...
        if self.isempty():
            self._num_verts_per_mesh = jt.zeros(
                (0,), dtype=jt.int64
            )
            self._num_faces_per_mesh = jt.zeros(
                (0,), dtype=jt.int64
            )
    ...
    def _set_verts_normals(self, verts_normals) -> None:
        if isinstance(verts_normals, list):
            ...
            for item, n_verts in zip(verts_normals, self._num_verts_per_mesh):
                if (
                    not isinstance(item, jt.Var)
                    or item.ndim != 2
                    or item.shape[1] != 3
                    or item.shape[0] != n_verts
                ):
                    raise ValueError("Invalid verts_normals input")
            self._verts_normals_packed = jt.cat(verts_normals, 0)
        elif jt.is_var(verts_normals):
            if (
    ...
    def __getitem__(
        self, index: Union[int, List[int], slice, jt.Var:jt.bool, jt.Var:jt.int32]
    ) -> "Meshes":
        """
        Args:
        ...
        if jt.is_var(verts) and jt.is_var(faces):
    ...
    def verts_padded_to_packed_idx(self):
        ...
        self._verts_padded_to_packed_idx = jt.cat(
            [
                jt.arange(v, dtype=jt.int64) + i * self._V
    ...
    def verts_normals_padded(self):
        ...
        """
        if self.isempty():
            return jt.zeros((self._N, 0, 3), dtype=jt.float32)
    ...
    def _compute_vertex_normals(self, refresh: bool = False):
        ...
        if self.isempty():
            self._verts_normals_packed = jt.zeros(
                (self._N, 3), dtype=jt.int64
            )
        else:
            faces_packed = self.faces_packed()
            verts_packed = self.verts_packed()
            verts_normals = jt.zeros_like(verts_packed)
            vertices_faces = verts_packed[faces_packed]

            faces_normals = jt.cross(
            ...
            verts_normals = verts_normals.index_add_(
            ...
            self._verts_normals_packed = jt.misc.normalize(
    ...
    def _compute_padded(self, refresh: bool = False):
        ...
        if self.isempty():
            self._faces_padded = jt.zeros(
                (self._N, 0, 3), dtype=jt.int64
            )
            self._verts_padded = jt.zeros(
                (self._N, 0, 3), dtype=jt.float32
            )
        else:
    ...
    def _compute_packed(self, refresh: bool = False):
        ...
        faces_list = self.faces_list()
        if self.isempty():
            self._verts_packed = jt.zeros(
                (0, 3), dtype=jt.float32
            )
            self._verts_packed_to_mesh_idx = jt.zeros(
                (0,), dtype=jt.int64
            )
            self._mesh_to_verts_packed_first_idx = jt.zeros(
                (0,), dtype=jt.int64
            )
            self._num_verts_per_mesh = jt.zeros(
                (0,), dtype=jt.int64
            ...
            self._faces_packed = -(
                torch.ones((0, 3), dtype=jt.int64)
            )
            self._faces_packed_to_mesh_idx = jt.zeros(
                (0,), dtype=jt.int64
            )
            self._mesh_to_faces_packed_first_idx = jt.zeros(
                (0,), dtype=jt.int64
            )
            self._num_faces_per_mesh = jt.zeros(
                (0,), dtype=jt.int64
        ...
        if not np.allclose(self.num_verts_per_mesh().numpy(), verts_list_to_packed[1].numpy()):
        ...
        if not np.allclose(self.num_faces_per_mesh().numpy(), faces_list_to_packed[1].numpy()):
    ...
    def _compute_edges_packed(self, refresh: bool = False):
        ...
        if self.isempty():
            self._edges_packed = jt.full(
                (0, 2), val=-1, dtype=jt.int64
            )
            self._edges_packed_to_mesh_idx = jt.zeros(
                (0,), dtype=jt.int64
            )
            return
        ...
        edges = jt.cat([e12, e20, e01], dim=0)  # (sum(F_n)*3, 2)
        edge_to_mesh = jt.cat(
            [
                self._faces_packed_to_mesh_idx,
                self._faces_packed_to_mesh_idx,
        ...
        u, inverse_idxs = jt.unique(edges_hash, return_inverse=True)
        ...
        sorted_hash, sort_idx = jt.argsort(edges_hash, dim=0)
        unique_mask = jt.ones(
            edges_hash.shape[0]
        ).bool()
        ...
        self._edges_packed = jt.stack([u // V, u % V], dim=1)
        self._edges_packed_to_mesh_idx = edge_to_mesh[unique_idx]

        self._faces_packed_to_edges_packed = inverse_idxs.reshape(3, F).transpose()

        # Compute number of edges per mesh
        num_edges_per_mesh = jt.zeros(self._N, dtype=jt.int32)
        ones = jt.ones(1, dtype=jt.int32).expand(
        ...
        mesh_to_edges_packed_first_idx = jt.zeros(
            self._N, dtype=jt.int64
        )
    ...
    def _compute_laplacian_packed(self, refresh: bool = False):
        ...
        if self.isempty():
            self._laplacian_packed = jt.zeros(
                (0, 0), dtype=jt.float32
            ).to_sparse()
            return
    ...
    def get_bounding_boxes(self):
        ...
        for verts in self.verts_list():
            cur_mins = verts.min(dim=0)  # (3,)
            cur_maxes = verts.max(dim=0)  # (3,)
    ...
    def submeshes(
        self,
        face_indices: Union[
            List[List[jt.Var:jt.int64]], List[jt.Var:jt.int64], jt.Var:jt.int64
        ],
    ) -> "Meshes":
        """
        ...
        for face_ids_per_mesh, faces, verts in zip(
            ...
            for submesh_face_ids in face_ids_per_mesh:
                ...
                vertex_ids_to_keep = jt.unique(faces_to_keep, sorted=True)
                ...
                _, ids_of_unique_ids_in_sorted = jt.unique(
...
def laplacian(verts: jt.Var, edges: jt.Var) -> jt.Var:
    """
    ...
    ones = torch.ones(idx.shape[1], dtype=jt.float32)
    # pyre-fixme[16]: Module `sparse` has no attribute `FloatTensor`.
